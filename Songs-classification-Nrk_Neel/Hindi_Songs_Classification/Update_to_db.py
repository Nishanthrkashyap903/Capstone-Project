...
# Load the JSON data
with open('Hindi_Merged_Songs_Features.json', 'r') as f:
...
for song_name, song_info in songs_data.items():
    ...
    if len(likelihoods) == 4:
        ...
        language = "Hindi"  # As per your use case, adjust if needed
        ...
        cursor.execute(insert_query, (
            song_name, 
            language, 
            likelihoods[2],  # Positive High Arousal
            likelihoods[0],  # Positive Low Arousal
            likelihoods[3],  # Negative High Arousal 
            likelihoods[1]  # Negative Low Arousal
        ))
